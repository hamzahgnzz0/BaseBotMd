...
async function Botstarted() {
    ...
    const clients = makeWASocket({
        ...
        browser: ['clients-BOTZ MD','Safari','1.1.0'],
        auth: state,
    ...
    clients.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./case")(clients, m, chatUpdate, store, opengc, antilink, antiwame, antilink2, antiwame2, set_welcome_db, set_left_db, set_proses, set_done, set_open, set_close, sewa, _welcome, _left, db_respon_list)
        } catch (err) {
            console.log(err)
        }
    })
...
try {
    ...
    clients.ev.on('creds.update', saveState)
